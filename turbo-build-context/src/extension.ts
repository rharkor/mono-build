...
import { resolvePackageOnly, resolveFromUri } from "./resolve";
import { runNpmBuildInDirectory, runTurboBuildDeps } from "./run";

function getTargetUri(resource: vscode.Uri | undefined): vscode.Uri | undefined {
...
function register(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(command, async (resource?: vscode.Uri) => {
      ...
      if (uri.scheme !== "file") {
        void vscode.window.showErrorMessage("Only file:// resources are supported.");
        return;
      }
      try {
        await run(uri);
...
export function activate(context: vscode.ExtensionContext): void {
  register(
    context,
    "turboBuildContext.buildPackage",
    async (uri) => {
  ...
  register(
    context,
    "turboBuildContext.buildPackageDeps",
    async (uri) => {
